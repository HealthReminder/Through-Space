...
public class SpaceTravelManager : MonoBehaviour {
	...
	void Start () {
		...
		//If this is the first level, spawn the Solar System prefab
		//Else
		//Spawn the player in a random direction
		player = Instantiate(pplayer, new Vector3(-50,-50,0), Quaternion.identity).transform.GetChild(0);
		...
		if(currentLevel < levels.Length && currentLevel >= 0){
			Vector3 newpos =  player.transform.position+player.transform.right*20;
			
			currentSolarSystem = Instantiate(levels[currentLevel].prefab, newpos ,Quaternion.identity).transform;
		}
		else {
